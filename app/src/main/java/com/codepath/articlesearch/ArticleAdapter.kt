...
class ArticleAdapter(private val context: Context, private val articles: List<Article>) :
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
        ...
        fun bind(article: Article) {
            titleTextView.text = article.headline?.main ?: "No Title"
            abstractTextView.text = article.abstract ?: "No Description"

            val imageUrl = article.mediaImageUrl
            if (imageUrl.isNotEmpty()) {
                Glide.with(context).load(imageUrl).into(mediaImageView)
            }
        }
